...
const Modal = ({ open, close, collectionId }) => {
    const { GetAllCollections, collections, collectionError, collectionSuccess, setCollectionError, setCollectionSuccess, DepositFunds } = useContext(CollectionContext);
    ...
    const sendFunds = (phaseId) => {
        if (!authenticated) {
            ...
            DepositFunds(phaseId, amount, bearerToken)//dodanie funduszy w pasku
        }
    }
    ...
    return open && collections[collectionId]['collectionType'] === 'STARTUP' ? (
        <div className='w-3/4 bg-indigo-900 shadow-lg shadow-indigo-900/50 fixed top-1/2 left-1/2 p-4 rounded-3xl -translate-x-1/2 -translate-y-1/2 z-10' ref={ref}>
            ...
            <div className='py-2 grid gap-5 grid-cols-2 grid-rows-1'>
                <div className='text-white text-md px-2 max-h-[680px] relative'>
                ...
                <div className='mx-2 text-white text-md px-2 max-h-[680px]'>
                {
