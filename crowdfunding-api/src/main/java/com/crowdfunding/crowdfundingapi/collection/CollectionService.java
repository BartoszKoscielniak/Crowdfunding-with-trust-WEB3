...
public class CollectionService {
    ...
    public ResponseEntity<List<Collection>> getAllCollections(CollectionType type, String name, Boolean excludeRequester) {
        ...
        if (name != null && type == null) {
            publishedCollections.stream().forEach(collection -> {
                if (collection.getCollectionName().toLowerCase().contains(name.toLowerCase())) {
                    response.add(collection);
                }
            });
            return ResponseEntity.status(HttpStatus.OK).body(response);
        ...
        if (name != null && type != null) {
            publishedCollections.stream().forEach(collection -> {
                if (collection.getCollectionName().toLowerCase().contains(name.toLowerCase()) && collection.getCollectionType() == type) {
                    response.add(collection);
                }
            });
            return ResponseEntity.status(HttpStatus.OK).body(response);
    ...
    public ResponseEntity<Map<String, String>> addCollection(String name, String description, CollectionType type,
        ...
        try {
            ...
            if (phase2name != null && phase2name.length() != 0 && phase2goal != 0 && phase2description.length() != 0 && phase2till.length() != 0 && phase2poe.length() != 0) {
            ...
            if (phase3name != null && phase3name.length() != 0 && phase3goal != 0 && phase3description.length() != 0 && phase3till.length() != 0 && phase3poe.length() != 0) {
            ...
            if (phase4name != null && phase4name.length() != 0 && phase4goal != 0 && phase4description.length() != 0 && phase4till.length() != 0 && phase4poe.length() != 0) {
    ...
    public ResponseEntity<Map<String, String>> updateCollection(Long id, String baseDescription) {
        Optional<Collection> optionalCollection = repository.findCollectionById(id);
        if (optionalCollection.isEmpty()) {
